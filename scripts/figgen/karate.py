...
# Hand-placed (F4 2026-08-25): clear space below-left of node 8, away from the
# Mr. Hi label and off the hub cluster; dotted leader disambiguates the target.
mx, my = 285.0, 302.0
PLACED.append((mx, my, len("the one miss") * 12.5 * 0.52))
# leader exits the label's top-right corner so the dots never cross the glyphs
mlx, mly = mx + 40.0, my - 7.0
lx0, ly0 = P[0]
lx33, ly33 = P[33]
...
<svg viewBox="0 0 760 400" role="img" aria-label="The real Zachary karate club network of 1977, colored by which side each member actually joined after the split; one ringed node is the only member the graph model mispredicted">
  ...
  <g stroke="var(--dkr-muted, #8e8e9a)" stroke-width="1.1" opacity="0.55">
    {edges_svg}
  </g>
  <g>
    {nodes}
  </g>
  <g stroke-width="1.4" stroke-dasharray="2,3" opacity="0.9" fill="none">
    <line x1="{hix}" y1="{hiy}" x2="{lx0}" y2="{ly0}" stroke="#b98a1e"/>
    <line x1="{ofx}" y1="{ofy}" x2="{lx33}" y2="{ly33}" stroke="var(--dkr-blue-dark, #0a5f56)"/>
    <line x1="{mlx}" y1="{mly}" x2="{lxm}" y2="{lym}" stroke="#cf4a30"/>
  </g>
